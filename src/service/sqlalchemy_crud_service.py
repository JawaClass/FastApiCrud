...
class SqlAlchemyCrudService(
    ...
    CrudService[TModel, TCreate, TUpdate, TResponse, ID, SaFilter[TModel], SqlAlchemyCrudContext],
):
    def __init__(
        self,
        repository: SqlAlchemyCrudRepository[TModel, ID],
        mapper: CrudServiceMapper[TModel, TCreate, TUpdate, TResponse],
    ):
        self._repository = repository
        self._mapper = mapper

    def create(self, context: SqlAlchemyCrudContext, data: TCreate) -> TResponse:
        entity = self._mapper.to_entity(data)
        saved = self._repository.save(context, entity)
        return self._mapper.to_response(saved)

    def create_all(self, context: SqlAlchemyCrudContext, data: list[TCreate]) -> list[TResponse]:
    ...
    def get_by_id(self, context: SqlAlchemyCrudContext, id: ID) -> TResponse | None:
    ...
    def get_all_by_id(self, context: SqlAlchemyCrudContext, ids: list[ID]) -> list[TResponse]:
    ...
    def update(self, context: SqlAlchemyCrudContext, id: ID, data: TUpdate) -> TResponse | None:
        entity = self._repository.find_by_id(context, id)
        if entity is None:
            return None
    ...
    def delete(self, context: SqlAlchemyCrudContext, id: ID) -> None:
        entity = self._repository.find_by_id(context, id)
    ...
    def delete_all_by_id(self, context: SqlAlchemyCrudContext, ids: list[ID]) -> None:
